...
// Placeholder palette — swap for real art assets by wiring
// APPEARANCE_ASSET_MAP in character-schema.ts.
...
export interface CharacterPortraitProps {
  ...
  mood: number;
  size?: number;      // rendered canvas size in CSS px
  className?: string;
  label?: string;
}

export function CharacterPortrait({
  appearance, mood, size = 256, className, label,
  ...
  useEffect(() => {
    ...
    draw(ctx, size, appearance, moodExpression(mood));
